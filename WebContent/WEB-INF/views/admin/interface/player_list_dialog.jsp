...
<script type="text/javascript">
$(function(){
	ListPage.enter({context:'#content',url:'${ctx}/admin/interface/queryPlayerJinQiu',searchForm:'#searchForm'},{type:1,league_id:'${league_id}',rel_palyer_id:'${rel_palyer_id}'});
})

function select_eventrecord(){
	var type = $("#select_type").val();
	var table = $("#eventrecord_table");
	var haveCheck = false;
	if(type=='2'){
	}else{
		table.find('input[type=radio]').each(function(){
			if(this.checked){
				haveCheck = true;
				window.parent.changeEventRecord(this.value);
			}
		});
	}
	if(!haveCheck){
		layer.msg('请选择联赛记录',{icon: 2});
	}
}
</script>
</body>
