...
<script type="text/javascript">
$(function(){
	ListPage.enter({context:'#content',url:'${ctx}/admin/user/selectUser',searchForm:'#searchForm'},{type:2});
})
...
function select_user(){
	...
	if(type=='2'){
		...
		if(haveCheck){
			...
			window.parent.saveRePlayer(uids,unames);
		}
	}else{
		table.find('input[type=radio]').each(function(){
			if(this.checked){
				haveCheck = true;
				window.parent.saveRePlayer(this.value,$(this).parent().parent().find("#uname").text());
			}
		});
	}
	if(!haveCheck){
