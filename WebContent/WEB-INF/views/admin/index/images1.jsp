                 ...
                 <div class="showPhotos">
                 	<c:forEach var="i" begin="1" end="22">
						...
						<c:if test="${i<=11}">
						    ...
						    <img src="${filePath}/${img.f_src}" sort="${i}" onclick="select_iv_info('${img.iid}','${i}')" class="ph${i}" />
							</c:if>
						    </c:forEach>
						    <c:if test="${flag1!=2}">
						    <img src="" sort="${i}" onclick="select_iv_info('','${i}')" class="ph${i}" />
							</c:if>
						</c:if>
						<c:if test="${i==11}">
						</div>
						...
						<c:if test="${i>11}">
						    ...
						    <img src="${filePath}/${img.f_src}" sort="${i}" onclick="select_iv_info('${img.iid}','${i}')" class="ph${i-11}" />
							</c:if>
						    </c:forEach>
						    <c:if test="${flag2!=2}">
						    <img src="" sort="${i}" onclick="select_iv_info('','${i}')" class="ph${i-11}" />
							</c:if>
						</c:if>
						<c:if test="${i==22}">
						</div>
