...
<script type="text/javascript">
$(function(){
	ListPage.enter({context:'#content',url:'${ctx}/admin/teamInfo/selectLeagueTeam',searchForm:'#searchForm'},
			{type:1,"league_id":'${league_id}',"group_id":'${group_id}'});
})
...
function select_team(){
	...
	if(type=='2'){
		...
		table.find('input[type=radio]').each(function(){
			if(this.checked){
				havaCheck = true;
				window.parent.changeTeam(this.value,$(this).parent().parent().find("#tname").text(),'${dom_id}','${dom_name}');
			}
		});
	}
	if(!havaCheck){
