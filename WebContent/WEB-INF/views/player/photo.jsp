                ...
                <div class="photo">
                    ...
                    <dl data-id="isme" class="mt20 ml30 pull-left">
                        ...
                        <dd id="click_btn" class="mt20">
                            <input type="button" flag="0" value="上传照片" class="uppicbtn" onclick="openUpPImgs('image')"/>
                        ...
                        <div id="photo_list" class="photo_div" style="margin-left: 30px;">
                            <div id="photo_model" class="item">
                                <div class="item_img">
                                    ...
                                    <a style="cursor: pointer;"><img data-id="f_src" to-oss="{{to_oss}}" layer-type="image" layer-pid="{{id}}" layer-src="${filePath}/{{f_src}}" alt="" num="" src="${filePath}/{{f_src}}"></a>
                                </div>
                            </div>
                            <div class="clearit"></div>
                        </div>
...
function load_photo_list(){
	photoList.loadListData().done(function(data){
		...
		if(data.state=='success'&&data.data.page.items.length>0){
			...
			$('#photo_list').empty().append('<span>没有上传图片</span>');
		}
		if(!data.data.isme){
			$('.photo').find('[data-id=isme]').remove();
			$('.photo').find('[data-tool]').remove();
...
function show_center(dom,id){
	...
	$.ajaxSec({
		...
		data: {id: id,type: "image",state:state},
		success: function(data){
			if(data.state != 'success'){
