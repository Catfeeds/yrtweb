          	...
          	<c:otherwise>
                ...
                <ul class="a_list">
                	<c:if test="${empty rf.items}">
                		<span style="color:#fff;">亲，还有没有竞拍纪录哦！</span> 
                	</c:if>
                            ...
                            <dl class="attr">
                                <dt><span class="ms">
                                	<c:choose>
	                                	...
	                                	<c:otherwise>
			                                ${fn:substringBefore((nowDate.time-auction.borndate.time)/1000/60/60/24/365,'.')} 岁</span>
	                                	</c:otherwise>
                                	</c:choose>
                                </span></dt>
                            ...
                            <div class="auc_div">
                            	<c:choose>
                            		<c:when test="${auction.status == 1}">
                           				<c:choose>
		                               		<c:when test="${auctionCondition.teaminfo_id eq auction.bidder}">
				                               <span>竞拍成功</span>
		                               		</c:when>
		                               		<c:otherwise>
											   <span class="text-red">竞拍失败</span>
		                               		</c:otherwise>
		                               	</c:choose>
                            		</c:when>
                            		<c:otherwise>
		...
		<ul class="pagination" style="float:right; margin-top:15px;margin-right: 40px;">
		    <c:choose>
				...
				<c:when test="${rf.currentPage!=1}"> 
					<li data-command="prev"><a href="javascript:void(0)" onclick="loadMyQueryPage(1)">首页</a></li>
					<li data-page-num="${rf.currentPage-1}"> <a href="javascript:void(0)" onclick="loadMyQueryPage(${rf.currentPage-1})">${rf.currentPage-1}</a></li>
					<li class="active"><a>${rf.currentPage}</a></li> 
				</c:when>
				<c:when test="${rf.currentPage==1}"> <li data-page-num="1" class="active"><a>1</a></li></c:when>
				</c:choose>
				<c:choose>
				<c:when test="${rf.currentPage!=rf.pageCount}"> <li data-page-num="${rf.currentPage+1}"><a href="javascript:void(0)" onclick="loadMyQueryPage(${rf.currentPage+1})">${rf.currentPage+1}</a></li>
				<c:choose>
				<c:when test="${(rf.currentPage+2)<rf.pageCount}"> <li><a>...</a></li> </c:when>
				</c:choose>
				<c:choose>
				<c:when test="${(rf.currentPage+1)!=rf.pageCount}"> <li data-page-num="${rf.pageCount}"><a href="javascript:void(0)"  onclick="loadMyQueryPage(${rf.pageCount})">${rf.pageCount}</a></li> </c:when>
				</c:choose>
				<li data-command="next"><a href="javascript:void(0)" onclick="loadMyQueryPage(${rf.pageCount})">末页</a></li> </c:when>
				</c:choose>
				</c:when>
			</c:choose>
		</ul>
	...
	$.ajaxSec({
		...
		success: function(data){
			if(data.state=='error'){
				...
				closeDetail();
				loadMyQueryPage($("#currentPage").val());
			}
		},
		error: $.ajaxError
	});	
