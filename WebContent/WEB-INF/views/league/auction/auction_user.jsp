...
<div class="player_cf">
    <c:choose>
		<c:when test="${!empty auctionMap.position}">
	 	<c:forEach items="${fn:split(auctionMap.position,',')}" var="pos" end="0">
			<c:choose>
	 			<c:when test="${auctionMap.level > 0}">
			 		<c:choose>
...
<div class="ability">
    ...
    <div style="width: 80%;margin: 0px auto;text-indent: 2.8em;line-height: 30px;">
        ...
        <span class="text-orange ml10 ms">
        	<c:choose>
          		<c:when test="${!empty auctionMap.bid_price}">
          			<fmt:formatNumber value="${auctionMap.bid_price}" pattern="#0"/>
          		</c:when>
          		<c:otherwise>
          			<fmt:formatNumber value="${auctionMap.init_price}" pattern="#0"/>
          		</c:otherwise>
          	</c:choose>	
        </span>
    ...
    <div style="width:256px;margin: 60px auto 0;text-align: center; ">
        <input type="button" name="name" value="提交" class="btn_l f18 ms" onclick="bid('${auctionMap.id}','${auctionMap.bidder}');"/>
