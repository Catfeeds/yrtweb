...
<c:choose>
	<c:when test="${!empty rf.items}">
		<table style="float: left; width: 100%;margin-top: 20px;">
             <tr>
                 <th class="w140"><span>头像</span></th>
                 <th><span>名字</span></th>
                 <th><span>常驻城市</span></th>
                 <th><span>寻找球员区域</span></th>
                 <th><span>所属公司</span></th>
             </tr>
		     <c:forEach items="${rf.items}" var="agent">
		     	 <tr>
			         <td>
			             <img src="${el:headPath()}${agent.head_icon}" class="ml35" width="40" height="40"/>
			         </td>
			         <td>
			             <span><a class="no" target="_blank" href="${ctx}/center/${agent.user_id}">${agent.usernick}</a></span>
			         </td>
			         <td>
			             <span>${agent.province}${agent.city}</span>
			         </td>
			         <td>
			             <span>${agent.find_area}</span>
			         </td>
			         <td>
