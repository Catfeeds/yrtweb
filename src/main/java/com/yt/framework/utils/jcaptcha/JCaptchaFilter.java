package com.yt.framework.utils.jcaptcha;
 ...
 * @Title: JCaptchaFilter.java 
 * @Package com.yt.framework.utils.jcaptcha
 * @author GL
 * @date 2015年8月21日 下午2:06:48 
 */
public class JCaptchaFilter implements Filter { 
    ...
    //默认值定义  
    public static final String DEFAULT_FILTER_PROCESSES_URL = "/doLogin";  
    public static final String DEFAULT_FILTER_PROCESSES_AJAX_URL = "/ajaxLogin";  
    ...
    public static final String DEFAULT_CAPTCHA_PARAMTER_Flag = "captcha_flag";  
    ...
    private String captchaParamterFlag = DEFAULT_CAPTCHA_PARAMTER_Flag; 
    ...
    public void doFilter(ServletRequest theRequest, ServletResponse theResponse,  
        ...
        if(StringUtils.startsWith(servletPath, DEFAULT_FILTER_PROCESSES_AJAX_URL)){
            ...
            if (validated) {  
                ...
                redirectFailureAjaxUrl(request, response);  
            } 
        }
        ...
        if(!StringUtils.startsWith(servletPath, filterProcessesUrl)&&!StringUtils.startsWith(servletPath, DEFAULT_FILTER_PROCESSES_AJAX_URL)){
        	genernateCaptchaImage(request, response);  
        }
    }  
    ...
    protected boolean validateCaptchaChallenge(final HttpServletRequest request) {  
        try {  
            ...
            String captchaFlag = request.getParameter(captchaParamterFlag);  
            logger.info("challengeResponse:"+challengeResponse);  
            //自动通过值存在时,检验输入值是否等于自动通过值  
            if(StringUtils.isNotBlank(captchaFlag) && captchaFlag.equals("true")){
            	return true;
            }
    ...
    protected void redirectFailureAjaxUrl(final HttpServletRequest request, final HttpServletResponse response)
    throws IOException {  
    	 response.sendRedirect(request.getContextPath() + "/ajaxLoginFailure");  
    }
