...
package com.yt.framework.utils.pushlet;
...
public class EventPullSources {
	 ...
	 * 获取系统消息
	 */
	static public class AuctionEventPullSource extends EventPullSource {
		...
		public Event pullEvent() {
			Event event = Event.createDataEvent("/auction");
			messageService = (MessageService)SpringContextUtil.getBean("messageService");
			List<Message> msgList = messageService.queryPointToPointMsg("", "", "system.info.auction.bidpass");
			try {  
				for(Message message:msgList){
					System.out.println("K"+message.getUser_id());
					
					//event.setField("K"+message.getUser_id(), URLEncoder.encode("测试","UTF-8"));
				}
			}catch (Exception e) {   
